...
func ParseRelayFromAck(ackNode *waBinary.Node) ParsedRelayAck {
	...
	for _, child := range wanode.NodeChildren(ackNode) {
		...
		parseRelayBlock(&child, &res, addParticipant)
	}
...
func parseRelayBlock(child *waBinary.Node, res *ParsedRelayAck, addParticipant func(string)) {
...
func FindRelayNode(n *waBinary.Node) *waBinary.Node {
	if n == nil {
		return nil
	}
	if n.Tag == "relay" {
	...
	children := wanode.NodeChildren(n)
	for i := range children {
		if found := FindRelayNode(&children[i]); found != nil {
			return found
		}
	}
	return nil
...
func ParseRelayFromNode(n *waBinary.Node) ParsedRelayAck {
	res := ParsedRelayAck{}
	relay := FindRelayNode(n)
	if relay == nil {
		return res
	}
	...
	parseRelayBlock(relay, &res, addParticipant)
	sortRelaysByRtt(res.Relays)
	return res
}
